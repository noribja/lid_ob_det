...
void ObstacleDetectorNode::lidarPointsCallback(const sensor_msgs::PointCloud2::ConstPtr& lidar_points)
{
  ...
  auto segmented_clouds = obstacle_detector->segmentPlane(raw_cloud, 30, GROUND_THRESH);
...
void ObstacleDetectorNode::publishDetectedObjects(std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr>&& cloud_clusters, const std_msgs::Header& header)
{
  ...
  if (USE_TRACKING)
    ...
    std::map<int, int> objectCounts = obstacle_detector->countObjectOccurrences(curr_boxes_);
    for (const auto& count : objectCounts) {
      std::cout << "Object with ID " << count.first << " detected " << count.second << " times." << std::endl;
    }
  ...
  for (auto& cluster : cloud_clusters){
    ...
    pcl::toROSMsg(*cluster, ros_cloud);
    autoware_objects.objects[i].pointcloud = ros_cloud;
    i = i + 1;
  }
...
int main(int argc, char** argv)
{
  ...
  ros::NodeHandle nh;
  bool stop_detecting;
  nh.param<bool>("/stop_detecting", stop_detecting, false);
  if (stop_detecting) {
      ROS_INFO("Stopping parameter is true. Shutting down the node.");
      ros::shutdown();
  }
  ros::spin();
  return 0;
